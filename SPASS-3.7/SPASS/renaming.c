...
static LIST ren_GetRenamings(TERM, TERM, int, FLAG_CNFRENAMINGTYPE);
static BOOL ren_HasBenefit(TERM, TERM, int, FLAG_CNFRENAMINGTYPE);
...
static BOOL ren_HasBenefit(TERM Term1, TERM Term2, int Pol, FLAG_CNFRENAMINGTYPE Type)
/**********************************************************
  INPUT:   Two terms, the polarity of the 2nd term in the overall formula
           and the type of renaming to be performed (opt, quant, complex).
  RETURNS: TRUE if renaming <Term1> in <Term2> results in a positive benefit.
  CAUTION: It is assumed that all superterms are set !
***********************************************************/
{ 
  if (Type != flag_CNFRENAMINGOFF) {

    if (Type == flag_CNFRENAMINGOPT) {
    ...
    if (Type ==  flag_CNFRENAMINGCOMPLEX) {
      SYMBOL Symbol;
      Symbol = term_TopSymbol(Term2);
      if (fol_IsQuantifier(Symbol) || 
	  symbol_Equal(Symbol, fol_AND) || symbol_Equal(Symbol, fol_OR) || 
	  symbol_Equal(Symbol, fol_IMPLIES) || symbol_Equal(Symbol, fol_EQUIV))
	return TRUE;
      else 
	return FALSE;
    }

    if (Type ==  flag_CNFRENAMINGQUANTIFIED) {
      if (fol_IsQuantifier(term_TopSymbol(Term2)))
	return TRUE;
      else 
	return FALSE;
    }
    ...
    misc_StartErrorReport();
    misc_ErrorReport("In ren_HasBenefit: Unknown Renaming Type.");
    misc_FinishErrorReport();
  }
...
static LIST ren_GetRenamings(TERM Term1, TERM Term2, int Pol, FLAG_CNFRENAMINGTYPE Type) 
/**********************************************************
  INPUT:   Two terms, the polarity of the 2nd term in the overall formula
           and the type of renaming to be performed (opt, quant, complex)
...
{ 
  ...
  if (term_Superterm(Term2) &&
      (Type != flag_CNFRENAMINGOPT || !fol_IsQuantifier(term_TopSymbol(term_Superterm(Term2)))) &&
      ren_HasBenefit(Term1, Term2, Pol, Type)) {
  ...
  if (fol_IsQuantifier(Top))
    Result = list_Nconc(Result,ren_GetRenamings(Term1, term_SecondArgument(Term2), Pol, Type));
    ...
    for (Scan=term_ArgumentList(Term2);!list_Empty(Scan);Scan=list_Cdr(Scan))
      Result = list_Nconc(Result,ren_GetRenamings(Term1,list_Car(Scan),Pol, Type));
  else if (symbol_Equal(Top,fol_Implies())) {
    Result = list_Nconc(Result,ren_GetRenamings(Term1,term_FirstArgument(Term2),-Pol, Type));
    Result = list_Nconc(Result,ren_GetRenamings(Term1,term_SecondArgument(Term2),Pol, Type));
  } else if (symbol_Equal(Top,fol_Equiv())) {
    Result = list_Nconc(Result, ren_GetRenamings(Term1,term_FirstArgument(Term2),0, Type));
    Result = list_Nconc(Result, ren_GetRenamings(Term1,term_SecondArgument(Term2),0, Type));
  } else {
    misc_StartErrorReport();
...
TERM ren_Rename(TERM Term, PRECEDENCE Precedence, LIST *SkolemSymbols,
		BOOL Document, BOOL Match, FLAG_CNFRENAMINGTYPE Type)
/**********************************************************
  INPUT:   A term, a precedence, a pointer to a list of
           Skolem symbols, a flag indicating whether the
	   renamings should be documented, a flag
	   indicating whether matching subterms should be
	   renamed using the same predicate and a flag
           indicating which type of renaming is performed (opt, complex, quant)
...
***********************************************************/
{
  ...
  Formulas = ren_GetRenamings(Term, Term, 1, Type);
